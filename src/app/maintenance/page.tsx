        ...
        <div className="p-6 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select value={type} onChange={e => setType(e.target.value)} className="w-full rounded-lg border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-gray-900">
            ...
            <div>
              ...
              <input type="date" value={dueDate} onChange={e => setDueDate(e.target.value)} className="w-full rounded-lg border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-gray-900"/>
            </div>
          </div>
          <div>
            <label className="block text-xs font-semibold text-gray-500 uppercase mb-1.5">Propriedade</label>
            <select value={propertyId} onChange={e => setPropertyId(e.target.value)} className="w-full rounded-lg border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-gray-900">
              {properties.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
            </select>
          </div>
          <div>
            <label className="block text-xs font-semibold text-gray-500 uppercase mb-1.5">Técnico responsável</label>
            <select value={assigneeId} onChange={e => setAssigneeId(e.target.value)} className="w-full rounded-lg border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-gray-900">
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <textarea rows={3} value={problem} onChange={e => setProblem(e.target.value)} disabled={isComplete}
          ...
          className="w-full rounded-lg border px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-gray-900 resize-none disabled:bg-gray-50"/>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <select value={actionTaken} onChange={e => setActionTaken(e.target.value)} disabled={isComplete}
            className="w-full rounded-lg border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-gray-900 disabled:bg-gray-50">
          ...
          {photoUrls.map((url, i) => (
            <div key={i} className="flex gap-2">
              <input value={url} onChange={e => setPhotoUrl(i, e.target.value)} disabled={isComplete}
                placeholder="https://…"
                className="flex-1 rounded-lg border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-gray-900 disabled:bg-gray-50 font-mono text-xs"/>
                      ...
                      {photoUrls.map((url, i) => (
                        <div key={i} className="flex gap-2">
                          <input value={url} onChange={e => setPhotoUrls(p => p.map((u, idx) => idx === i ? e.target.value : u))}
                            placeholder="https://…"
                            className="flex-1 rounded-lg border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-gray-900 font-mono text-xs"/>
                          {photoUrls.length > 1 && (
