    ...
    <div className="flex h-[calc(100vh-80px)] max-h-[750px] rounded-xl border bg-white overflow-hidden">
      ...
      <aside className="w-64 border-r flex flex-col shrink-0">
        ...
        <div className="flex-1 overflow-y-auto">
          {myConvs.length > 0 && (
            <>
              <div className="px-4 py-2 text-xs text-gray-400 uppercase font-semibold tracking-wide">Os meus clientes</div>
              {myConvs.map(c => (
          ...
          {otherConvs.length > 0 && (
            <>
              <div className="px-4 py-2 text-xs text-gray-400 uppercase font-semibold tracking-wide mt-1">Outros gestores</div>
              {otherConvs.map(c => (
                <button key={c.id} onClick={() => { setActive(c); setText('') }}
        ...
        <div className="flex flex-col flex-1 min-w-0">
          ...
          {!active.readonly ? (
            ...
            <div className="flex items-center justify-center gap-2 px-4 py-3 border-t bg-gray-50 text-xs text-gray-400">
              <Lock className="h-3.5 w-3.5"/>Conversa de outro gestor — só leitura
            </div>
          )}
        </div>
      ) : (
