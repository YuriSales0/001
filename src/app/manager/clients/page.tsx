...
export default function ManagerClients() {
  ...
  useEffect(() => { fetch('/api/users?role=CLIENT').then(r => r.ok ? r.json() : []).then(setClients) }, [])
  return (
    <div className="p-6 space-y-4">
      ...
      <div className="rounded-xl border bg-white overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {clients.length === 0 && <tr><td colSpan={4} className="text-center py-8 text-gray-500">No clients assigned</td></tr>}
            {clients.map(c => (
